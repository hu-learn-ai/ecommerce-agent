...
#   说明：抽检表是**双盲**的（不含 Judge 分数），且给的是**模型真实回答**
#   （reports/predictions/<model>.jsonl）+ 数据集参考答案；
#   填法与 1-5 分锚点见 finetune/reports/human_scores_README.md。
#   只想验证脚本链路可用（不产生真实结论）：--selftest

# 结论（2026-09-15，50 抽 49 有效）：
#   最终（修正多轮输入口径 + 按定义重打 safety 后）：总分一致率 0.755 / kappa 0.536 / ±1 内 0.939 /
#   偏差 -0.082（=人工-Judge，负值指 Judge 更宽松）；kappa 的 95% 自助法区间 [0.31, 0.74]（n=49）。
#   同一批题的演进：0.449/0.157（原始口径）→ 0.674/0.420（修多轮口径）→ 0.755/0.536（对齐 safety 定义）。
#   全程只改评估口径、没动模型——说明前两个低值主要来自评估方法本身。
#   分轮数看：单轮 kappa 0.397、多轮 0.865；
#   分类目看：闲聊 1.00、投诉安抚 1.00、政策规则 0.67、物流 0.57、售后退换 0.46、域外拒答 0.43、
#   售前咨询 0.25、退款发票 0.18（后两类是 Judge 偏宽最明显的地方）。
#   Safety 维度：人工与 Judge 都近乎只给 5（人工 5:42、Judge 5:48），**该维度已无区分度**，
#   kappa 在这种情形下失效，引用时用一致率 0.837 / PABAK 0.674 并说明"模型在安全上几乎不出错"。
#   因此：judge_means / win_rate 只能作相对比较并需注明 Judge 未过人工校验；
#   deterministic（政策引用、OOD 拒答、多轮保持）是规则口径，不受影响。

# 复核结论（2026-09-15，对 reports_run_a 逐条核对）：
#   ✅ 可溯源：数据流水线（320 种子 → 15,320 → 清洗 10,660 → 9,594/533/289）、防泄漏移除 244 条、
#      训练 3 epoch / 897 步 / train_loss 0.73、三项规则指标与四维总分、成对胜率口径（胜=差值≥1 分）。
#   ✅ 我重算了测试集↔训练集相似度：78.5% 的测试首问与训练首问余弦 ≥0.8，且**无 ≥0.9**（防泄漏阈值已生效）。
#   ⚠️ **幻觉率规则假阳性偏高**：微调 54 条里 52 条是"回答提到政策库其他条目的措辞"被判幻觉，
#      而微调回答更贴政策原文、也更短（62 字 vs base 251 字 / deepseek 405 字）。逐条核对后，
#      按"数字与政策冲突"口径：微调 2/289 = 0.7% < base 10/289 = 3.5% —— **原来的"幻觉是短板"结论站不住**。
#      结论已改为"该规则假阳性偏高、待人工复核"，引用时不要再说"幻觉率翻倍"。
#   ⚠️ 样本量提示：政策引用 n=212、多轮保持 n=114（且为 LLM 布尔判定、已到 100% 天花板）、域外拒答 n=33。
#   ⚠️ `reports/`（09-10 22:59）评的是 **5k 消融模型**，`reports_run_a/`（09-11 00:39）才是全量模型；
#      对外引用请用 **reports_run_a**（两者对比：政策引用 52.3% vs 78.5%、OOD 拒答 72.7% vs 87.9%）。
